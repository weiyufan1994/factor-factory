...
Step3B must implement the route frozen by the mechanism-conditioned measurement program, and BLOCK if correctness cannot be proven. It must not try a fixed operator-first sequence or silently change routes after failure. UBL/CPV/shadow/candle/Williams logic is allowed only as an explicit family plugin or fixture. Unsupported operator parity, missing `formula_ir`, unsafe direct code, or ambiguous proxy rewrites are BLOCK conditions, not warnings.
