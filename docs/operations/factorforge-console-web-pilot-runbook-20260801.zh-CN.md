...
- OpenClaw `replayInvalid=true` 只表示工具副作用使本轮不可安全重放；成功终态仍应为 `livenessState=working`。`abandoned/blocked/paused`、非空 `error`、`isError=true` 或 `incomplete_turn` 必须阻断。
...
- 首次研究表单只暴露研究描述、可选公式/代码、可选 PDF 和折叠后的样本设置。第三方算子的歧义选择、语义权威、来源哈希与实现证明不得要求用户填写；Host 必须在任何回测前使用版本化的预注册研究口径冻结相关选择，并明确 `source_truth_claimed=false`。客户端提交同名内部字段不能覆盖 Host policy。
- PDF 仅接受单个不超过 20 MiB、最多 600 页且可提取文本的文件；Pilot 不提供 OCR 或加密 PDF 密码处理。Web 将原件写入 workspace 外的私有附件账本，API 只返回文件名、大小和 SHA-256，不返回服务器路径。Runner 核验账本哈希后，才把原件和带页码文本写入当前 factor workspace；原件、提取文本和 manifest 必须作为只读研究输入进入组织计划与 Agent 容器。部分落盘、哈希变化、symlink、跨 workspace 路径或恢复时 provenance 不一致均须 fail closed。
- 普通 Web agent 只读取任务内 `web_research_runtime.md`、请求、catalog 摘要、factor knowledge 摘要、Host 生成的 `web_research_authoring_contract.json` 和研究计划，不递归读取整套 skill/validator；fresh turn 只写 Formula IR 计划和短 ledger，且必须原样保留模板中 Host 已填的 `identity` 与 `authoring_contract` 引用，并在退出前运行无数据访问、无物化副作用的 authoring preflight，直到计划校验 PASS；引用错误时 preflight 返回精确期望值，不让 Agent 猜测或重算哈希。普通 resume turn 只写当前 pause 点名的 memo。`web_execution_ledger.md` 必须存在，独立 completion 文件仅为旧任务兼容写入、不得作为必需门槛；Host 根据已验证计划、ledger 和私有 agent-run receipt 推导 authoring 状态。Agent 不运行 materializer、Step、Council merge/finalize 或 Ultimate，preflight PASS 也不构成正式研究 proof。
...
- workspace 虽为任务可写，但 `manifest.json`、原始请求、conversation ledger 目录、catalog 摘要、factor knowledge 摘要、authoring contract、runtime guide、agent prompt 和用户假设在 agent 容器内必须以独立只读 bind mount 覆盖；Host 必须对 agent 前后文件哈希做 allowlist 差异校验，任何正式 artifact 或未授权路径变化都命中 `BLOCK_FACTORFORGE_CONSOLE_AGENT_WRITE_SCOPE_INVALID`。
- Agent 可见的 catalog 摘要只投影 Web v1 Formula IR 真正支持的 `clean_daily_bar`，避免把完整生产 catalog 注入模型上下文；完整 catalog SHA 与只读文件仍由 Host 保存并在正式 materializer/Ultimate 阶段独立校验和使用。摘要中省略的数据集不等于 Data API 缺失，也不得触发 Agent 网络探测。
- Agent 容器与 Host formal 子进程统一设置 `PYTHONDONTWRITEBYTECODE=1`，authoring preflight 还必须使用 `python3 -B`；任何 `__pycache__` 仍按 workspace 外写入处理并由隔离审计 BLOCK，不加入忽略或白名单。
