...
def _account(
    ...
    calendar_dates: list[str], *, account_id: str, engine_source: str, simulation_pair=None,
) -> dict[str, object]:
    net_nav, trades = simulation_pair[0] if simulation_pair is not None else engine._simulate(
    ...
    del trades
    gross_nav, gross_trades = simulation_pair[1] if simulation_pair is not None else engine._simulate(
        windows, dict(selections), daily_lookup, execution_lookup, constraint_lookup, calendar_dates,
        cost_rate=0.0,
    )
...
def evaluate_monthly_groups_from_normalized(
    *,
    ...
    selections_by_account = {}
    for number in range(1, GROUP_COUNT + 1):
        group_id = f"G{number:02d}"
        selections_by_account[group_id] = {
    ...
    selections_by_account["BENCHMARK_ALL_TEN_GROUPS"] = {
        formation: (
    ...
    synchronized = callable(getattr(execution_engine, '_simulate_many', None))
    requests = [{'key': (account_id, label), 'selections': selections, 'cost_rate': cost}
        for account_id, selections in selections_by_account.items()
        for label, cost in [('net', ONE_WAY_COST), ('gross', 0.0)]]
    results = execution_engine._simulate_many(portfolio_windows, requests, daily_lookup,
        execution_lookup, constraint_lookup, calendar_dates) if synchronized else None
    group_accounts, account_summaries, benchmark = {}, {}, None
    try:
        for account_id, selections in selections_by_account.items():
            pair = None
            if results is not None:
                net_key, net_nav, net_trades = next(results)
                gross_key, gross_nav, gross_trades = next(results)
                if net_key != (account_id, 'net') or gross_key != (account_id, 'gross'):
                    raise MonthlyEvaluationError('synchronized_account_result_order_mismatch')
                pair = ((net_nav, net_trades), (gross_nav, gross_trades))
            account = _account(execution_engine, portfolio_windows, selections, daily_lookup,
                execution_lookup, constraint_lookup, calendar_dates, account_id=account_id,
                engine_source=engine_source, simulation_pair=pair)
            retained, summary = finalize_account(account_id, account)
            account_summaries[account_id] = summary
            if account_id == "BENCHMARK_ALL_TEN_GROUPS":
                benchmark = retained
            elif retained is not None:
                group_accounts[account_id] = retained
            del account, pair
            if synchronized:
                del net_nav, net_trades, gross_nav, gross_trades
    finally:
        if results is not None:
            results.close()
    return {
