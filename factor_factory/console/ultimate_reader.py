...
def _research_notebooks(
    *,
    ...
    if measurement.get("contract_version") == MEASUREMENT_PROGRAM_VERSION_V2:
        # V2 classifies the research equation as a first-class public object.
        # V1 deliberately retains its historical notebook shape.
        math_notebook["research_equation"] = _public_copy(
            measurement.get("research_equation")
        ) or {}
...
def _measurement_program_knowledge_node_ids(value: dict[str, Any]) -> set[str]:
...
def _public_research_equation_copy(value: Any) -> dict[str, Any]:
    equation = _public_allowed_dict(value, PUBLIC_RESEARCH_EQUATION_FIELDS)
    if not equation or not isinstance(value, dict):
        return {}
    equation["validity_scope"] = _public_allowed_dict(
        value.get("validity_scope"),
        PUBLIC_RESEARCH_EQUATION_VALIDITY_SCOPE_FIELDS,
    )
    equation["participant_constraint_loop"] = _public_allowed_dict(
        value.get("participant_constraint_loop"),
        PUBLIC_PARTICIPANT_CONSTRAINT_LOOP_FIELDS,
    )
    equation["equation_quality"] = _public_allowed_dict(
        value.get("equation_quality"),
        PUBLIC_EQUATION_QUALITY_FIELDS,
    )
    return equation
...
def _public_measurement_program_copy(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict):
        return {}
    is_v2 = value.get("contract_version") == MEASUREMENT_PROGRAM_VERSION_V2
    available_node_ids = _measurement_program_knowledge_node_ids(value)
    if is_v2 and validate_measurement_program(
        value,
    ...
    for section_name, allowed in PUBLIC_MEASUREMENT_SECTION_FIELDS.items():
        ...
        elif is_v2 and section_name == "evaluation_design":
            # ``return_path`` is a closed portfolio-semantics field in the V2
            # measurement contract, not a filesystem locator.  The generic
            # public-copy denylist intentionally removes keys ending in
            # ``_path``; restore this one schema-authorized field so the V2
            # projection remains exactly revalidatable.  V1 keeps its legacy
            # projection behavior unchanged.
            source_portfolio = (
                section.get("portfolio_contract")
                if isinstance(section, dict)
                else None
            )
            copied_portfolio = copied_section.get("portfolio_contract")
            if (
                isinstance(source_portfolio, dict)
                and isinstance(copied_portfolio, dict)
                and "return_path" in source_portfolio
            ):
                copied_portfolio["return_path"] = _public_copy(
                    source_portfolio["return_path"]
                )
        output[section_name] = copied_section
    if is_v2:
        output["research_equation"] = _public_research_equation_copy(
            value.get("research_equation")
        )
        if validate_measurement_program(
            output,
            available_knowledge_node_ids=(
                _measurement_program_knowledge_node_ids(output)
            ),
            require_web_executable=False,
        ):
            return {}
