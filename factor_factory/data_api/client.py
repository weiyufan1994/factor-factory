...
def _ensure_independent_data_api() -> None:
    try:
        ...
        candidates = [
            ...
            Path("/opt/factorforge/workspace/factorforge-data-api"),
            Path("/Users/researcher/projects/factorforge-data-api"),
        ]
...
def _hardened_pyarrow_s3_transport():
    """Compatibility context: SDK constructs configured filesystems per call.

    No process-global Arrow constructor is changed, including concurrent reads.
    """
    yield
...
def _catalog_item(catalog_path: Path | None, dataset_id: str) -> dict[str, Any]:
    return _catalog_item_from_payload(_read_json(catalog_path), dataset_id)


def _catalog_item_from_payload(catalog: dict, dataset_id: str) -> dict[str, Any]:
...
def _normalize_result_metadata(result: Any, catalog_path: Path | None, dataset_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "catalog_binding": (payload.get("delivery_contract") or {}).get("catalog_binding") or source,
...
def fetch_data_api_dataset(
    ...
    *,
    ...
    query = DataQuery(
        ...
        expected_trade_dates=expected_trade_dates, calendar_identity=calendar_identity, minute_domain=minute_domain,
    )
    with _hardened_pyarrow_s3_transport():
        return IndependentDataApiClient.from_catalog(catalog, expected_sha256=catalog_sha256, read_policy=read_policy).fetch(query)
...
def resolve_data_api_dataset(
    ...
    *,
    ...
    catalog_item = _catalog_item_from_payload(catalog_payload, dataset_id)
    if not catalog_item:
        return _blocked_metadata(
            dataset_id,
    ...
    return {
        "catalog_binding": {"catalog_path": str(catalog.resolve()),
            "catalog_source": str(catalog.resolve()),
            "catalog_sha256": hashlib.sha256(catalog_bytes).hexdigest(),
            "catalog_version": catalog_payload.get("catalog_version", "factorforge_data_catalog_v1"),
            "dataset_version": catalog_item.get("version")},
        "dataset_id": dataset_id,
