...
def resolve_field(name: str, available_columns: Iterable[str] | None = None) -> str:
    ...
    if _adv_token(key) and ('volume' in lower_to_actual or 'vol' in lower_to_actual):
        return key
    if key == 'vwap' and 'amount' in lower_to_actual and ('volume' in lower_to_actual or 'vol' in lower_to_actual):
        return 'vwap'
    raise KeyError(f'BLOCK_MISSING_FIELD_ALIAS: {name}')
