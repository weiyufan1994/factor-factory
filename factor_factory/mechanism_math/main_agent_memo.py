...
PUBLIC_MEMO_MODEL_LAYER_REVIEW_FIELDS = frozenset(
    {
        "equation_supported_by_metrics",
        "failed_equation_component",
        "mechanism_fit",
        "return_source",
        "interpretation",
    }
)
PUBLIC_MEMO_MODEL_LAYER_REVIEW_NEGATIVE_ENUMS = {
    "equation_supported_by_metrics": frozenset({"challenged"}),
    "failed_equation_component": frozenset(
        {
            "assumptions",
            "math_tool_selection",
    ...
    "mechanism_fit": frozenset({"contradicted"}),
    "return_source": frozenset({"unknown"}),
}
...
def _single_expectation_projection_suffix_is_safe(value: str) -> bool:
    ...
    expected_named_end: int | str | None = None
        ...
        for index_part in candidate_parts:
            ...
            if timing_match:
                ...
                if parsed_start is not None and parsed_end is not None:
                    ...
                    # A single named return horizon (for example r_{i,t+20})
                    # is a legitimate payoff declaration when its explicit
                    # entry/exit clause binds the exit to that horizon.  It is
                    # not an invitation to infer an unstated holding period.
                    expected_named_end = parsed_start
                break
    suffix = text[end + 1 :]
    if not suffix.strip():
        return False
        ...
        if timing_match:
            ...
            if expected_entry is not None and expected_exit is not None:
                ...
                if (
                    ...
                    return False
            elif expected_named_end is not None:
                if (
                    exit_offset != expected_named_end
                    or normalized_entry_field != normalized_exit_field
                    or isinstance(entry_offset, int)
                    and isinstance(exit_offset, int)
                    and exit_offset <= entry_offset
...
def _direct_code_symbol_names(value: str) -> set[str] | None:
    """Return bounded identifier names from a direct-code declaration.

    This is deliberately not an evaluator or a parser for arbitrary Python.  It
    is only used to bind the public, declarative direct-code component chain to
    the memo's displayed formula components when Formula-IR is unavailable.
    """
    text = str(value or "")
    try:
        if not text.strip() or len(text.encode("utf-8")) > 8_192:
            return None
    except UnicodeError:
        return None
    if _has_future_temporal_reference(text) or "\\x00" in text:
        return None
    return {
        token.casefold()
        for token in re.findall(r"(?<![a-z0-9_])[a-z][a-z0-9_]*", text.casefold())
        if not _information_name_has_future_semantics(token)
    }


def _direct_code_assignment_names(value: str) -> set[str] | None:
    symbols = _direct_code_symbol_names(value)
    if symbols is None:
        return None
    return {
...
def _validated_direct_code_measurement_program_projection_aliases(
    memo: dict[str, Any],
    factor_spec: dict[str, Any],
) -> set[str]:
    """Bind a current-time measured object through a validated V2 direct path.

    Formula-IR is the root oracle for operator/hybrid declarations.  A V2
    direct-code program intentionally need not be Formula-IR parseable, so it
    receives this separate, fail-closed route.  The route is anchored to the
    two identical program copies in the factor spec, revalidates that program,
    and requires its selected-primary-object -> terminal-projection component
    chain to be visibly present in the memo formula components.  It never
    treats prose in a caller memo as a substitute for a validated program.
    """
    program, failures = _validated_measurement_program_from_factor_spec(
        factor_spec
    )
    if failures or not isinstance(program, dict):
        return set()
    if program.get("contract_version") != (
        "factorforge_mechanism_conditioned_measurement_program_v2"
    ):
        return set()
    implementation = program.get("implementation")
    if not isinstance(implementation, dict) or implementation.get("route") != "direct_code":
        return set()
    canonical = _canonical(factor_spec)
    declared_modes = [
        container.get("implementation_mode")
        for container in (factor_spec, canonical)
        if isinstance(container, dict) and container.get("implementation_mode") is not None
    ]
    if not declared_modes or any(mode != "direct_code" for mode in declared_modes):
        return set()

    selected = _selected_measurement_program_model(program)
    projection = program.get("market_outcome_projection")
    observation = program.get("observation_and_estimation")
    if not all(isinstance(value, dict) for value in (selected, projection, observation)):
        return set()
    normalized = lambda value: re.sub(r"\s+", " ", str(value or "")).strip().casefold()
    if (
        not normalized(selected.get("mathematical_object"))
        or normalized(selected.get("mathematical_object"))
        != normalized(projection.get("source_math_object"))
        or normalized(selected.get("observation_mapping"))
        != normalized(observation.get("observation_map"))
    ):
        return set()

    math_payload = memo.get("math_hypothesis")
    object_mapping = memo.get("mathematical_object_mapping")
    components = memo.get("formula_component_map")
    if not all(isinstance(value, dict) for value in (math_payload, object_mapping)) or not isinstance(components, list):
        return set()
    ...
    if (
        ...
        or normalized(math_object) != normalized(object_mapping.get("mathematical_object"))
        or normalized(math_observation) != normalized(object_mapping.get("observation_mapping"))
        or _contains_future_payoff_object(math_object)
        or _has_future_temporal_reference(math_observation)
        or _current_object_alias_names(math_object) is None
        or "measured_object" not in _current_object_alias_names(math_object)
    ):
        return set()
    ...
    for component in components:
        if not isinstance(component, dict):
            return set()
        component_id = component.get("component_id")
        expression = component.get("formula_subexpression")
        if (
            not isinstance(component_id, str)
            or not component_id.strip()
            or component_id in component_by_id
            or _direct_code_symbol_names(str(expression or "")) is None
        ):
            return set()
        component_by_id[component_id] = component
    links = object_mapping.get("component_links")
    if (
        not isinstance(links, list)
        or not links
        or any(not isinstance(link, str) or not link.strip() for link in links)
        or len(links) != len(set(links))
        or set(links) != set(component_by_id)
    ):
        return set()

    program_components = implementation.get("components")
    if not isinstance(program_components, list):
        return set()
    primary = [
        component for component in program_components
        if isinstance(component, dict)
        and component.get("binding_role") == "selected_primary_object"
    ]
    terminal = [
        component for component in program_components
        if isinstance(component, dict)
        and component.get("binding_role") == "terminal_market_projection"
    ]
    if len(primary) != 1 or len(terminal) != 1:
        return set()
    primary_formula = str(primary[0].get("math_term_or_functional") or "")
    terminal_formula = str(terminal[0].get("math_term_or_functional") or "")
    primary_symbols = _direct_code_symbol_names(primary_formula)
    terminal_symbols = _direct_code_symbol_names(terminal_formula)
    primary_assignments = _direct_code_assignment_names(primary_formula)
    terminal_assignments = _direct_code_assignment_names(terminal_formula)
    memo_symbols = _direct_code_symbol_names(
        ";".join(
            [str(memo.get("formula") or "")]
            + [str(component.get("formula_subexpression") or "") for component in components]
        )
    )
    if (
        primary_symbols is None
        or terminal_symbols is None
        or primary_assignments is None
        or terminal_assignments is None
        or memo_symbols is None
        or not terminal_assignments <= memo_symbols
        or not (primary_assignments & terminal_symbols & memo_symbols)
    ):
        return set()
    # The selected-primary-object component is the measurement boundary.  The
    # terminal projection legitimately names T+1 execution, which is checked
    # separately by the market-projection grammar and must not be mistaken for
    # feature-time lookahead.
    for field in (
        ...
        if _has_future_temporal_reference(str(primary[0].get(field) or "")):
            return set()
    return {"measured_object"}


def _validated_v1_direct_code_measurement_program_projection_aliases(
    memo: dict[str, Any],
    factor_spec: dict[str, Any],
) -> set[str]:
    """Bind a frozen V1 direct-code component program without Formula-IR.

    V1 direct-code programs predate V2's selected-primary/terminal roles.  The
    replacement authority is therefore deliberately narrower: every displayed
    component must be an exact display of the validated program's component,
    and the frozen formula must equal the executable formula projection.  This
    is a binding check only; it does not validate the model's mechanism claim.
    """
    program, failures = _validated_measurement_program_from_factor_spec(
        factor_spec
    )
    ...
    frozen_formula = _formula_text(factor_spec)
    observation = program.get("observation_and_estimation")
    selected = _selected_measurement_program_model(program)
    market_projection = program.get("market_outcome_projection")
    if not frozen_formula or not all(
        isinstance(value, dict) for value in (observation, selected, market_projection)
    ):
        return set()
    normalized = lambda value: re.sub(r"\s+", " ", str(value or "")).strip().casefold()
    executable = str(observation.get("executable_formula_projection") or "")
    program_observation = str(observation.get("observation_map") or "")
    selected_projection = str(selected.get("market_outcome_projection") or "")
    if (
        str(memo.get("formula") or "") != frozen_formula
        or executable != frozen_formula
        or not program_observation
        or not selected_projection
        or normalized(selected.get("observation_mapping"))
        != normalized(program_observation)
        or normalized(selected.get("mathematical_object"))
        != normalized(market_projection.get("source_math_object"))
    ):
        return set()

    math_payload = memo.get("math_hypothesis")
    object_mapping = memo.get("mathematical_object_mapping")
    memo_components = memo.get("formula_component_map")
    if not all(isinstance(value, dict) for value in (math_payload, object_mapping)) or not isinstance(memo_components, list):
        return set()
    math_object = str(math_payload.get("mathematical_object") or "").strip()
    memo_observation = str(math_payload.get("observation_mapping") or "").strip()
    if (
        not math_object
        or normalized(math_object) != normalized(object_mapping.get("mathematical_object"))
        or normalized(memo_observation) != normalized(object_mapping.get("observation_mapping"))
        or normalized(memo_observation) != normalized(program_observation)
        or _contains_future_payoff_object(math_object)
        or _has_future_temporal_reference(memo_observation)
        or _current_object_alias_names(math_object) is None
        or "measured_object" not in _current_object_alias_names(math_object)
    ):
        return set()
    ...
    program_components = implementation.get("components")
    if not isinstance(program_components, list) or not program_components:
        return set()
    ...
    for component in program_components:
        if not isinstance(component, dict):
            return set()
        component_id = component.get("component_id")
        formula = str(component.get("math_term_or_functional") or "")
        observable = str(component.get("observable_or_input") or "")
        if (
            not isinstance(component_id, str)
            or not component_id.strip()
            or component_id in frozen_by_id
            or not observable
            or _direct_code_symbol_names(formula) is None
        ):
            return set()
        frozen_by_id[component_id] = component

    memo_by_id: dict[str, dict[str, Any]] = {}
    for component in memo_components:
        if not isinstance(component, dict):
            return set()
        component_id = component.get("component_id")
        if (
            not isinstance(component_id, str)
            or not component_id.strip()
            or component_id in memo_by_id
        ):
            return set()
        memo_by_id[component_id] = component
    links = object_mapping.get("component_links")
    if (
        not isinstance(links, list)
        or not links
        or any(not isinstance(link, str) or not link.strip() for link in links)
        or len(links) != len(set(links))
        or set(links) != set(frozen_by_id)
        or set(memo_by_id) != set(frozen_by_id)
    ):
        return set()
    for component_id, frozen in frozen_by_id.items():
        displayed = memo_by_id[component_id]
        if (
            str(displayed.get("formula_subexpression") or "")
            != str(frozen.get("math_term_or_functional") or "")
            or str(displayed.get("observable_estimator") or "")
            != str(frozen.get("observable_or_input") or "")
            or _direct_code_symbol_names(
                str(displayed.get("formula_subexpression") or "")
            )
            is None
        ):
...
def _bound_measured_object_projection_aliases(
    ...
    direct_code_aliases = _validated_direct_code_measurement_program_projection_aliases(
        memo,
        factor_spec,
    )
    if direct_code_aliases:
        return direct_code_aliases
    v1_direct_code_aliases = (
        _validated_v1_direct_code_measurement_program_projection_aliases(
            memo,
            factor_spec,
        )
    )
    if v1_direct_code_aliases:
        return v1_direct_code_aliases
    math_payload = (
        memo.get("math_hypothesis")
        if isinstance(memo.get("math_hypothesis"), dict)
        else {}
...
def memo_public_schema_failures(memo: dict[str, Any]) -> list[str]:
    ...
    if isinstance(refs, dict):
        if "model_layer_review" in refs:
            model_layer_review = refs["model_layer_review"]
            failures.extend(
                _unexpected_fields(
                    model_layer_review,
                    PUBLIC_MEMO_MODEL_LAYER_REVIEW_FIELDS,
                    "evidence_comparison.model_layer_review",
                )
            )
            if not isinstance(model_layer_review, dict):
                failures.append(
                    "BLOCK_MAIN_AGENT_MECHANISM_MEMO_PUBLIC_FIELD_TYPE:"
                    "evidence_comparison.model_layer_review"
                )
            else:
                for key, allowed in PUBLIC_MEMO_MODEL_LAYER_REVIEW_NEGATIVE_ENUMS.items():
                    value = model_layer_review.get(key)
                    if value is None:
                        failures.append(
                            "BLOCK_MAIN_AGENT_MECHANISM_MEMO_MODEL_LAYER_REVIEW_FIELD_MISSING:"
                            f"{key}"
                        )
                    elif not isinstance(value, str):
                        failures.append(
                            "BLOCK_MAIN_AGENT_MECHANISM_MEMO_PUBLIC_FIELD_TYPE:"
                            f"evidence_comparison.model_layer_review.{key}"
                        )
                    elif value not in allowed:
                        failures.append(
                            "BLOCK_MAIN_AGENT_MECHANISM_MEMO_MODEL_LAYER_REVIEW_NEGATIVE_ENUM:"
                            f"{key}"
                        )
                interpretation = model_layer_review.get("interpretation")
                if not isinstance(interpretation, str) or not interpretation.strip():
                    failures.append(
                        "BLOCK_MAIN_AGENT_MECHANISM_MEMO_PUBLIC_FIELD_TYPE:"
                        "evidence_comparison.model_layer_review.interpretation"
                    )
...
def _legacy_factor_spec_knowledge_node_ids(
    factor_spec: dict[str, Any] | None,
) -> set[str]:
    """Project the pre-Step2-summary knowledge layout without widening it."""
    factor_spec = factor_spec if isinstance(factor_spec, dict) else {}
    canonical = (
        factor_spec.get("canonical_spec")
        if isinstance(factor_spec.get("canonical_spec"), dict)
...
def _has_current_step2_knowledge_summary(
    factor_spec: dict[str, Any],
) -> bool:
    """Identify the explicit Step2 summary containers, not a legacy contract.

    ``research_contract`` alone remains on the older strict intersection route:
    its context was historically coupled to an independently validated retrieval
    contract.  The newer summary carries method/context and materialized-case
    lanes under ``research_discipline`` or ``learning_and_innovation`` and must
    use the shared Step2 projection instead.
    """
    canonical = factor_spec.get("canonical_spec")
    containers = [factor_spec]
    if isinstance(canonical, dict):
        containers.append(canonical)
    return any(
        isinstance(container.get(key), dict)
        for container in containers
        for key in ("research_discipline", "learning_and_innovation")
    )
...
def _factor_spec_knowledge_node_ids(
    ...
    """Return Step2-authorized node IDs for memo program validation.

    This projects an already-frozen Step2 summary; it is not a retrieval or
    authorization admission path.  In particular, component declarations and
    bare cited-ID lists are never inputs to the current-summary route.
    """
    factor_spec = factor_spec if isinstance(factor_spec, dict) else {}
    if _has_current_step2_knowledge_summary(factor_spec):
        # A malformed current summary fails closed rather than falling back to
        # a looser-looking legacy field elsewhere in the same object.
        return measurement_program_available_knowledge_node_ids_from_step2_summary(
            factor_spec
        )
    return _legacy_factor_spec_knowledge_node_ids(factor_spec)
...
def formula_specific_derivation_from_main_agent_memo(memo: dict[str, Any], factor_spec: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    evidence_comparison = (
        memo.get("evidence_comparison")
        if isinstance(memo.get("evidence_comparison"), dict)
        else {}
    )
    observed_evidence = {
        field: evidence_comparison.get(field)
        for field in (
            "observed_metrics",
            "mechanism_supported",
            "contradictions",
        ...
        if field in evidence_comparison
    }
    observed_evidence_text = (
        json.dumps(observed_evidence, ensure_ascii=False, sort_keys=True)
        if observed_evidence
        else ""
    )
    triggered_kill_criteria = evidence_comparison.get("kill_criteria_triggered")
    if isinstance(triggered_kill_criteria, list) and any(
        isinstance(item, str) and item.strip()
        for item in triggered_kill_criteria
    ):
        kill_criteria = [
            item.strip()
            for item in triggered_kill_criteria
        ...
        # The downstream derivation contract requires two kill criteria.  If a
        # memo recorded a single triggered criterion, retain it verbatim and
        # use a separately recorded contradiction before falling back to the
        # legacy prospective criteria.
        contradictions = evidence_comparison.get("contradictions")
        if len(kill_criteria) < 2 and isinstance(contradictions, list):
            kill_criteria.extend(
                item.strip()
                for item in contradictions
                if isinstance(item, str)
                and item.strip()
                and item.strip() not in kill_criteria
            )
    ...
    revision_implications = evidence_comparison.get("revision_implications")
    if isinstance(revision_implications, list) and any(
        isinstance(item, str) and item.strip()
        for item in revision_implications
    ):
        ...
        revision_implication = (
            "Use only formula/model mutation after a specific answered derivation "
            "step is contradicted; do not repair through portfolio construction."
        )
    ...
    return {
        ...
        "observed_metric_comparison": (
            observed_evidence_text
            or str(qa.get("metric_signature_answer") or "")
        ),
        "metric_feedback_to_model": (
            observed_evidence_text
            or str(qa.get("falsification_answer") or "")
        ),
        "falsification_tests": falsification_tests,
        "kill_criteria": kill_criteria,
        "revision_implication": revision_implication,
    }
