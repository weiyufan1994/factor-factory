...
APPEND_TMP_ROOT="${APPEND_TMP_ROOT:-${ROOT}/intraday_flow_distribution_moments_v1_oos_append_tmp}"
...
APPEND_START="${APPEND_START:-20260526}"
APPEND_END="${APPEND_END:-20260526}"
...
HORIZONS="${HORIZONS:-1}"
LOG_PATH="${LOG_PATH:-${ROOT}/moneyflow_oos_feature_validation_append_validate_worker.log}"

mkdir -p "${RESULT_DIR}"
exec > >(tee -a "${LOG_PATH}") 2>&1
trap 'rc=$?; echo "APPEND_VALIDATE_FAILED_STAGE=${CURRENT_STAGE:-unknown} LINE=${LINENO} RC=${rc}"; exit ${rc}' ERR
stage() {
  CURRENT_STAGE="$1"
  date -u +"APPEND_VALIDATE_STAGE_${CURRENT_STAGE}_%Y-%m-%dT%H:%M:%SZ"
}

stage append_missing_distribution_moments
python3 "${DATA_API_REPO}/scripts/build_intraday_datamarts.py" \
  --catalog "${CATALOG}" \
  --output-root "${APPEND_TMP_ROOT}" \
  --start "${APPEND_START}" \
  --end "${APPEND_END}" \
  --dataset intraday_flow_distribution_moments_v1 \
  --prior-threshold-root "${PRIOR_THRESHOLD_ROOT}" \
  --qa-output "${ROOT}/intraday_flow_distribution_moments_v1_oos.append.qa.json" \
  --catalog-output "${ROOT}/intraday_flow_distribution_moments_v1_oos.append.catalog.json" \
  --is-end-date "${IS_END_DATE}" \
  --chunk-days 1 \
  --overwrite

stage copy_appended_partitions
for part in "${APPEND_TMP_ROOT}"/trade_date=*; do
  [[ -d "${part}" ]] || continue
  target="${MOMENTS_ROOT}/$(basename "${part}")"
  rm -rf "${target}"
  cp -R "${part}" "${target}"
  echo "APPENDED_PARTITION=${target}"
done
...
stage write_append_validate_summary
python3 - "${ROOT}" "${RESULT_DIR}" "${APPEND_START}" "${APPEND_END}" "${FULL_START}" "${OOS_END}" <<'PY'
import json
import sys
...
payload = {
    "verdict": "ACCEPT",
    "scope": "research_side_oos_full_v18a_append_validate",
    "append_start": sys.argv[3],
    "append_end": sys.argv[4],
    "full_start": sys.argv[5],
    "oos_end": sys.argv[6],
    "out_root": str(root),
...
(root / "moneyflow_oos_feature_validation_full_v18a_append_validate_summary.json").write_text(
...
aws s3 cp --only-show-errors "${ROOT}/moneyflow_oos_feature_validation_full_v18a_append_validate_summary.json" "${RESULT_S3}/"
aws s3 cp --only-show-errors "${ROOT}/intraday_flow_distribution_moments_v1_oos.append.qa.json" "${RESULT_S3}/"
aws s3 cp --only-show-errors "${ROOT}/intraday_flow_distribution_moments_v1_oos.append.catalog.json" "${RESULT_S3}/"
date -u +"APPEND_VALIDATE_DONE_%Y-%m-%dT%H:%M:%SZ"
