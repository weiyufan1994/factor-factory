...
V19_SCRIPT_S3="${V19_SCRIPT_S3:?Set the explicit read-only V19 script URI}"
V18_SCRIPT_S3="${V18_SCRIPT_S3:?Set the explicit read-only V18 script URI}"
BASELINE_SCRIPT_S3="${BASELINE_SCRIPT_S3:?Set the explicit read-only baseline script URI}"
VP_S3="${VP_S3:?Set the explicit read-only VP datamart URI}"
INDEX_UNIVERSE_S3="${INDEX_UNIVERSE_S3:?Set the explicit read-only index-universe URI}"
VP_ROOT="${VP_ROOT:?Set an isolated local VP datamart root}"
INDEX_UNIVERSE_ROOT="${INDEX_UNIVERSE_ROOT:?Set an isolated local index-universe root}"
MINUTE_ROOT="${MINUTE_ROOT:?Set an isolated local minute-cache root}"
FLOW_FEATURE_ROOT="${FLOW_FEATURE_ROOT:?Set an isolated local flow-feature root}"
FLOW_FEATURE_S3="${FLOW_FEATURE_S3:?Set the explicit read-only flow-feature URI}"
SKIP_FLOW_BUILD="${SKIP_FLOW_BUILD:-0}"
DAILY_CLEAN="${DAILY_CLEAN:-${WORKSPACE}/data/clean/daily_clean.parquet}"
DAILY_S3="${DAILY_S3:?Set the explicit read-only daily-clean URI}"
OUT_DIR="${OUT_DIR:?Set an isolated local output directory}"
UPLOAD_S3="${UPLOAD_S3:-}"
