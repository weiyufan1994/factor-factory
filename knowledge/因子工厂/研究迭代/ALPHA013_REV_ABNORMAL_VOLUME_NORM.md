---
report_id: "ALPHA013_REV_ABNORMAL_VOLUME_NORM"
factor_id: "Alpha013AbnormalVolumeNorm"
decision: "iterate"
iteration_no: 2
tags:
  - "iteration"
...
# Research Iteration: Alpha013AbnormalVolumeNorm (ALPHA013_REV_ABNORMAL_VOLUME_NORM)
...
- `cost_adjusted_annual_return`: `-0.09659506550131032`

- `cost_adjusted_long_side_max_drawdown`: `-0.8991931555423339`

- `cost_adjusted_long_side_recovery_days`: `3803`

- `cost_adjusted_long_side_sharpe`: `-0.4065939917641699`

- `cost_adjusted_return_daily`: `-0.0003833137519893267`

- `group_bottom_decile_mean_return`: `-0.0007314501246699569`

- `group_long_short_spread_ir`: `0.3263954800877856`

- `group_long_short_spread_mean`: `0.0013996927657552627`

- `group_top_decile_mean_return`: `0.0006682426410853057`

- `long_side_annual_return`: `0.16843921195146477`

- `long_side_annual_volatility`: `0.2376491140758002`

- `long_side_max_drawdown`: `-0.3988456246460056`

- `long_side_mean_return_daily`: `0.0006684095712359713`

- `long_side_recovery_days`: `1762`

- `long_side_return_std_daily`: `0.014970487027128257`

- `long_side_sharpe`: `0.7087727324653088`

- `long_side_turnover_mean_daily`: `0.3506636003937858`

- `metric_period`: `daily`

- `pearson_ic_ir`: `0.3977922583101702`

- `pearson_ic_mean`: `0.021558373417365793`

- `rank_ic_ir`: `0.5224250433386033`

- `rank_ic_mean`: `0.0313680745315596`

- `trading_cogs_annual`: `0.26510168189770206`

- `trading_cogs_daily`: `0.0010519908011813574`

- `turnover_mean`: `0.3506636003937858`
...
- rank_ic_mean=0.031368 is positive, so the raw cross-sectional ordering contains directional information.
- rank_ic_ir=0.522 is usable for a first-pass daily factor, but not strong enough to ignore robustness checks.
- long-side highest-score group mean return=0.168439 is positive; this is revenue evidence but no longer sufficient alone for adoption.
- long-side Sharpe=0.709 clears the candidate threshold 0.50, but still needs more evidence for official admission.
- volatility-drag adjusted growth proxy=0.140201 is positive under mean - 0.5*sigma^2.
...
- recovery_days=1762 is longer than the soft payback limit; the factor may not survive its drawdown cycle.
...
- group long-short spread mean=0.001400 is positive, but this is diagnostic only because short selling and direct decile trading are not allowed.
- group long-short spread IR=0.326 is positive, but cannot justify adoption without long-side evidence.
...
- [[普通因子库/ALPHA013_REV_ABNORMAL_VOLUME_NORM|Factor Record]]

- [[知识库/ALPHA013_REV_ABNORMAL_VOLUME_NORM|Knowledge Record]]
