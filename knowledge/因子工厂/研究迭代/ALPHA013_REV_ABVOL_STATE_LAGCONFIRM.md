---
report_id: "ALPHA013_REV_ABVOL_STATE_LAGCONFIRM"
factor_id: "Alpha013AbvolStateLagConfirm"
decision: "iterate"
iteration_no: 1
tags:
  - "iteration"
...
# Research Iteration: Alpha013AbvolStateLagConfirm (ALPHA013_REV_ABVOL_STATE_LAGCONFIRM)
...
- backend_statuses: `{'qlib_backtest': 'partial', 'self_quant_analyzer': 'success'}`

## Evidence Metrics

- `annualization_factor`: `252`

- `cost_adjusted_annual_return`: `-0.04050269759284935`

- `cost_adjusted_long_side_max_drawdown`: `-0.8327744811329545`

- `cost_adjusted_long_side_recovery_days`: `3662`

- `cost_adjusted_long_side_sharpe`: `-0.1731015532024196`

- `cost_adjusted_return_daily`: `-0.00016072499044781488`

- `group_bottom_decile_mean_return`: `-0.000720166519322088`

- `group_long_short_spread_ir`: `0.2755178179731202`

- `group_long_short_spread_mean`: `0.0012635187157627797`

- `group_top_decile_mean_return`: `0.0005433521964406918`

- `long_side_annual_return`: `0.1363624973113571`

- `long_side_annual_volatility`: `0.23412258454778176`

- `long_side_max_drawdown`: `-0.4371463338434297`

- `long_side_mean_return_daily`: `0.0005411210210768139`

- `long_side_recovery_days`: `3179`

- `long_side_return_std_daily`: `0.014748336548026762`

- `long_side_sharpe`: `0.5824405944208559`

- `long_side_turnover_mean_daily`: `0.23400824490864452`

- `metric_period`: `daily`

- `pearson_ic_ir`: `0.29408569148807734`

- `pearson_ic_mean`: `0.017458225519521232`

- `rank_ic_ir`: `0.3797726471366032`

- `rank_ic_mean`: `0.0253997369648826`

- `trading_cogs_annual`: `0.17691023315093526`

- `trading_cogs_daily`: `0.0007020247347259336`

- `turnover_mean`: `0.23400824490864452`
...
- rank_ic_mean=0.025400 is positive, so the raw cross-sectional ordering contains directional information.
- rank_ic_ir=0.380 is usable for a first-pass daily factor, but not strong enough to ignore robustness checks.
- long-side highest-score group mean return=0.136362 is positive; this is revenue evidence but no longer sufficient alone for adoption.
- long-side Sharpe=0.582 clears the candidate threshold 0.50, but still needs more evidence for official admission.
- volatility-drag adjusted growth proxy=0.108956 is positive under mean - 0.5*sigma^2.
...
- recovery_days=3179 is longer than the soft payback limit; the factor may not survive its drawdown cycle.
...
- group long-short spread mean=0.001264 is positive, but this is diagnostic only because short selling and direct decile trading are not allowed.
- group long-short spread IR=0.276 is positive, but cannot justify adoption without long-side evidence.
...
- [[普通因子库/ALPHA013_REV_ABVOL_STATE_LAGCONFIRM|Factor Record]]

- [[知识库/ALPHA013_REV_ABVOL_STATE_LAGCONFIRM|Knowledge Record]]
