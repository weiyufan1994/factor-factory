---
report_id: "ALPHA013_REV_ABVOL_STATE_MEAN5"
factor_id: "Alpha013AbnormalVolumeStateMean5"
decision: "iterate"
iteration_no: 2
tags:
  - "iteration"
...
# Research Iteration: Alpha013AbnormalVolumeStateMean5 (ALPHA013_REV_ABVOL_STATE_MEAN5)
...
- `cost_adjusted_annual_return`: `-0.036144706912525075`

- `cost_adjusted_long_side_max_drawdown`: `-0.8332762413686079`

- `cost_adjusted_long_side_recovery_days`: `3662`

- `cost_adjusted_long_side_sharpe`: `-0.1541853571611731`

- `cost_adjusted_return_daily`: `-0.00014343137663700426`

- `group_bottom_decile_mean_return`: `-0.0007403794321734779`

- `group_long_short_spread_ir`: `0.28781075728776867`

- `group_long_short_spread_mean`: `0.00131785916837591`

- `group_top_decile_mean_return`: `0.0005774797362024321`

- `long_side_annual_return`: `0.1456943905593489`

- `long_side_annual_volatility`: `0.23455695092745737`

- `long_side_max_drawdown`: `-0.42803707855029516`

- `long_side_mean_return_daily`: `0.0005781523434894798`

- `long_side_recovery_days`: `1834`

- `long_side_return_std_daily`: `0.014775699057991266`

- `long_side_sharpe`: `0.6211471882767122`

- `long_side_turnover_mean_daily`: `0.24058914090096367`

- `metric_period`: `daily`

- `pearson_ic_ir`: `0.31435926242899515`

- `pearson_ic_mean`: `0.018612363931867753`

- `rank_ic_ir`: `0.4070406517685194`

- `rank_ic_mean`: `0.027056581100298984`

- `trading_cogs_annual`: `0.18188539052112854`

- `trading_cogs_daily`: `0.000721767422702891`

- `turnover_mean`: `0.24058914090096367`
...
- rank_ic_mean=0.027057 is positive, so the raw cross-sectional ordering contains directional information.
- rank_ic_ir=0.407 is usable for a first-pass daily factor, but not strong enough to ignore robustness checks.
- long-side highest-score group mean return=0.145694 is positive; this is revenue evidence but no longer sufficient alone for adoption.
- long-side Sharpe=0.621 clears the candidate threshold 0.50, but still needs more evidence for official admission.
- volatility-drag adjusted growth proxy=0.118186 is positive under mean - 0.5*sigma^2.

## Weaknesses
- qlib backend is not yet consistently successful
- max_drawdown=-0.428 breaches the soft capital-cost limit -0.35; reduce drawdown before promotion.
- recovery_days=1834 is longer than the soft payback limit; the factor may not survive its drawdown cycle.
...
- group long-short spread mean=0.001318 is positive, but this is diagnostic only because short selling and direct decile trading are not allowed.
- group long-short spread IR=0.288 is positive, but cannot justify adoption without long-side evidence.
...
- [[普通因子库/ALPHA013_REV_ABVOL_STATE_MEAN5|Factor Record]]

- [[知识库/ALPHA013_REV_ABVOL_STATE_MEAN5|Knowledge Record]]
