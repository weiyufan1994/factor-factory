---
report_id: "ALPHA015_ADAPTIVE_B_FAST7_HALFSLOW14_20160101"
factor_id: "Alpha015"
decision: "iterate"
iteration_no: 1
tags:
...
# Research Iteration: Alpha015 (ALPHA015_ADAPTIVE_B_FAST7_HALFSLOW14_20160101)
...
- `rank_ic_mean`: `0.03762107754134773`

- `rank_ic_ir`: `0.3837411281612381`

- `pearson_ic_mean`: `0.02708270519732972`

- `pearson_ic_ir`: `0.31760207986175193`

- `group_long_short_spread_mean`: `0.0020608755039840244`

- `group_long_short_spread_ir`: `0.3137923471752357`

- `group_top_decile_mean_return`: `0.00040781100073237976`

- `group_bottom_decile_mean_return`: `-0.0016530645032516449`

- `metric_period`: `daily`

- `annualization_factor`: `252`

- `long_side_mean_return_daily`: `0.00040781730804126853`

- `long_side_annual_return`: `0.10276996162639968`

- `long_side_return_std_daily`: `0.013339205442868454`

- `long_side_annual_volatility`: `0.2117533217341748`

- `long_side_sharpe`: `0.485328687100419`

- `long_side_max_drawdown`: `-0.4140781860203745`

- `long_side_recovery_days`: `1754`

- `long_side_turnover_mean_daily`: `0.13885525615186953`

- `turnover_mean`: `0.13885525615186953`

- `trading_cogs_daily`: `0.0004165657684556086`

- `trading_cogs_annual`: `0.10497457365081336`

- `cost_adjusted_return_daily`: `-8.580218989277191e-06`

- `cost_adjusted_annual_return`: `-0.002162215185297852`

- `cost_adjusted_long_side_sharpe`: `-0.01021171612302793`

- `cost_adjusted_long_side_max_drawdown`: `-0.6424636278400452`
...
- rank_ic_mean=0.037621 is positive, so the raw cross-sectional ordering contains directional information.
- rank_ic_ir=0.384 is usable for a first-pass daily factor, but not strong enough to ignore robustness checks.
- long-side highest-score group mean return=0.102770 is positive; this is revenue evidence but no longer sufficient alone for adoption.
- volatility-drag adjusted growth proxy=0.080350 is positive under mean - 0.5*sigma^2.

## Weaknesses
- current run is still partial rather than fully validated
- qlib backend is not yet consistently successful
- long-side Sharpe=0.485 is below candidate threshold 0.50
- long-side Sharpe=0.485 is below the candidate threshold 0.50; improve risk-adjusted performance rather than raw return only.
- max_drawdown=-0.414 breaches the soft capital-cost limit -0.35; reduce drawdown before promotion.
- recovery_days=1754 is longer than the soft payback limit; the factor may not survive its drawdown cycle.
...
- group long-short spread mean=0.002061 is positive, but this is diagnostic only because short selling and direct decile trading are not allowed.
- group long-short spread IR=0.314 is positive, but cannot justify adoption without long-side evidence.
...
- [[普通因子库/ALPHA015_ADAPTIVE_B_FAST7_HALFSLOW14_20160101|Factor Record]]

- [[知识库/ALPHA015_ADAPTIVE_B_FAST7_HALFSLOW14_20160101|Knowledge Record]]
