---
report_id: "ALPHA015_REV_CORR5_SUM5_20160101"
factor_id: "Alpha015"
decision: "iterate"
iteration_no: 1
tags:
...
# Research Iteration: Alpha015 (ALPHA015_REV_CORR5_SUM5_20160101)
...
- `rank_ic_mean`: `0.04144924149265613`

- `rank_ic_ir`: `0.5356203289691014`

- `pearson_ic_mean`: `0.028912539222713713`

- `pearson_ic_ir`: `0.4156580976271314`

- `group_long_short_spread_mean`: `0.0020800394655757217`

- `group_long_short_spread_ir`: `0.37874671641797153`

- `group_top_decile_mean_return`: `0.0004156645845344803`

- `group_bottom_decile_mean_return`: `-0.0016643748810412416`

- `metric_period`: `daily`

- `annualization_factor`: `252`

- `long_side_mean_return_daily`: `0.0004159128896978379`

- `long_side_annual_return`: `0.10481004820385516`

- `long_side_return_std_daily`: `0.013760478270260341`

- `long_side_annual_volatility`: `0.21844082054650266`

- `long_side_sharpe`: `0.47980980817430474`

- `long_side_max_drawdown`: `-0.40234715757771844`

- `long_side_recovery_days`: `1708`

- `long_side_turnover_mean_daily`: `0.2746939391643187`

- `turnover_mean`: `0.2746939391643187`

- `trading_cogs_daily`: `0.0008240818174929562`

- `trading_cogs_annual`: `0.20766861800822495`

- `cost_adjusted_return_daily`: `-0.00040783850204632383`

- `cost_adjusted_annual_return`: `-0.1027753025156736`

- `cost_adjusted_long_side_sharpe`: `-0.4706014015089389`

- `cost_adjusted_long_side_max_drawdown`: `-0.8269179878257098`
...
- rank_ic_mean=0.041449 is positive, so the raw cross-sectional ordering contains directional information.
- rank_ic_ir=0.536 is usable for a first-pass daily factor, but not strong enough to ignore robustness checks.
- long-side highest-score group mean return=0.104810 is positive; this is revenue evidence but no longer sufficient alone for adoption.
- volatility-drag adjusted growth proxy=0.080952 is positive under mean - 0.5*sigma^2.

## Weaknesses
- current run is still partial rather than fully validated
- qlib backend is not yet consistently successful
- long-side Sharpe=0.480 is below candidate threshold 0.50
- long-side Sharpe=0.480 is below the candidate threshold 0.50; improve risk-adjusted performance rather than raw return only.
- max_drawdown=-0.402 breaches the soft capital-cost limit -0.35; reduce drawdown before promotion.
- recovery_days=1708 is longer than the soft payback limit; the factor may not survive its drawdown cycle.
...
- group long-short spread mean=0.002080 is positive, but this is diagnostic only because short selling and direct decile trading are not allowed.
- group long-short spread IR=0.379 is positive, but cannot justify adoption without long-side evidence.
...
- [[普通因子库/ALPHA015_REV_CORR5_SUM5_20160101|Factor Record]]

- [[知识库/ALPHA015_REV_CORR5_SUM5_20160101|Knowledge Record]]
