---
report_id: "ALPHA015_REV_OUTER_SMOOTH5_20160101"
factor_id: "Alpha015"
decision: "iterate"
iteration_no: 1
tags:
...
# Research Iteration: Alpha015 (ALPHA015_REV_OUTER_SMOOTH5_20160101)
...
- `rank_ic_mean`: `0.0354845452165256`

- `rank_ic_ir`: `0.5050386502131501`

- `pearson_ic_mean`: `0.024516157738375167`

- `pearson_ic_ir`: `0.38780669782077776`

- `group_long_short_spread_mean`: `0.0017523236183652274`

- `group_long_short_spread_ir`: `0.35301941606545284`

- `group_top_decile_mean_return`: `0.00031732973074719456`

- `group_bottom_decile_mean_return`: `-0.001434993887618033`

- `metric_period`: `daily`

- `annualization_factor`: `252`

- `long_side_mean_return_daily`: `0.0003174846857941315`

- `long_side_annual_return`: `0.08000614082012113`

- `long_side_return_std_daily`: `0.013670019238182974`

- `long_side_annual_volatility`: `0.21700482793020467`

- `long_side_sharpe`: `0.3686836905115011`

- `long_side_max_drawdown`: `-0.4131286562616928`

- `long_side_recovery_days`: `1790`

- `long_side_turnover_mean_daily`: `0.2562646201524626`

- `turnover_mean`: `0.2562646201524626`

- `trading_cogs_daily`: `0.0007687938604573878`

- `trading_cogs_annual`: `0.1937360528352617`

- `cost_adjusted_return_daily`: `-0.000451000917301405`

- `cost_adjusted_annual_return`: `-0.11365223115995406`

- `cost_adjusted_long_side_sharpe`: `-0.5237694608534492`

- `cost_adjusted_long_side_max_drawdown`: `-0.8347008295496948`
...
- rank_ic_mean=0.035485 is positive, so the raw cross-sectional ordering contains directional information.
- rank_ic_ir=0.505 is usable for a first-pass daily factor, but not strong enough to ignore robustness checks.
- long-side highest-score group mean return=0.080006 is positive; this is revenue evidence but no longer sufficient alone for adoption.
- volatility-drag adjusted growth proxy=0.056461 is positive under mean - 0.5*sigma^2.

## Weaknesses
- current run is still partial rather than fully validated
- qlib backend is not yet consistently successful
- long-side Sharpe=0.369 is below candidate threshold 0.50
- long-side Sharpe=0.369 is below the candidate threshold 0.50; improve risk-adjusted performance rather than raw return only.
- max_drawdown=-0.413 breaches the soft capital-cost limit -0.35; reduce drawdown before promotion.
- recovery_days=1790 is longer than the soft payback limit; the factor may not survive its drawdown cycle.
...
- group long-short spread mean=0.001752 is positive, but this is diagnostic only because short selling and direct decile trading are not allowed.
- group long-short spread IR=0.353 is positive, but cannot justify adoption without long-side evidence.
...
- [[普通因子库/ALPHA015_REV_OUTER_SMOOTH5_20160101|Factor Record]]

- [[知识库/ALPHA015_REV_OUTER_SMOOTH5_20160101|Knowledge Record]]
