---
report_id: "ALPHA015_RISK_A_HIGHAMOUNT_LOWVOL20_20160101"
factor_id: "Alpha015"
decision: "iterate"
iteration_no: 1
tags:
...
# Research Iteration: Alpha015 (ALPHA015_RISK_A_HIGHAMOUNT_LOWVOL20_20160101)
...
- `rank_ic_mean`: `0.029047126145642012`

- `rank_ic_ir`: `0.2572694183999949`

- `pearson_ic_mean`: `0.020351005410113282`

- `pearson_ic_ir`: `0.2087195280189173`

- `group_long_short_spread_mean`: `0.0012007617907628162`

- `group_long_short_spread_ir`: `0.13875409000472552`

- `group_top_decile_mean_return`: `0.00015440987922818631`

- `group_bottom_decile_mean_return`: `-0.0010463519115346297`

- `metric_period`: `daily`

- `annualization_factor`: `252`

- `long_side_mean_return_daily`: `0.00015488454240951383`

- `long_side_annual_return`: `0.039030904687197485`

- `long_side_return_std_daily`: `0.016004890762202045`

- `long_side_annual_volatility`: `0.2540697643052497`

- `long_side_sharpe`: `0.1536227846470711`

- `long_side_max_drawdown`: `-0.6493287209761822`

- `long_side_recovery_days`: `3440`

- `long_side_turnover_mean_daily`: `0.25288029090207087`

- `turnover_mean`: `0.25288029090207087`

- `trading_cogs_daily`: `0.0007586408727062126`

- `trading_cogs_annual`: `0.1911774999219656`

- `cost_adjusted_return_daily`: `-0.0006034507963165513`

- `cost_adjusted_annual_return`: `-0.15206960067177092`

- `cost_adjusted_long_side_sharpe`: `-0.59855507033952`

- `cost_adjusted_long_side_max_drawdown`: `-0.9112905604776349`
...
- rank_ic_mean=0.029047 is positive, so the raw cross-sectional ordering contains directional information.
- long-side highest-score group mean return=0.039031 is positive; this is revenue evidence but no longer sufficient alone for adoption.
- volatility-drag adjusted growth proxy=0.006755 is positive under mean - 0.5*sigma^2.

## Weaknesses
- current run is still partial rather than fully validated
- qlib backend is not yet consistently successful
- long-side Sharpe=0.154 is below candidate threshold 0.50
- long-side Sharpe=0.154 is below the candidate threshold 0.50; improve risk-adjusted performance rather than raw return only.
- max_drawdown=-0.649 breaches the soft capital-cost limit -0.35; reduce drawdown before promotion.
- recovery_days=3440 is longer than the soft payback limit; the factor may not survive its drawdown cycle.

## Risks
- rank_ic_ir=0.257 is positive but weak; it needs regime and turnover checks before promotion.
...
- group long-short spread mean=0.001201 is positive, but this is diagnostic only because short selling and direct decile trading are not allowed.
- group long-short spread IR=0.139 is only marginally positive.
...
- [[普通因子库/ALPHA015_RISK_A_HIGHAMOUNT_LOWVOL20_20160101|Factor Record]]

- [[知识库/ALPHA015_RISK_A_HIGHAMOUNT_LOWVOL20_20160101|Knowledge Record]]
