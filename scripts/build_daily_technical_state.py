...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Build read-only daily technical state features and QA proof.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    params = DailyTechnicalStateParams(volume_col=str(args.volume_col), amount_col=str(args.amount_col))
    source = pd.read_parquet(Path(args.input_parquet).expanduser())
    if 'trade_date' not in source.columns:
        raise SystemExit('daily technical state build requires trade_date column')
    ...
    available_dates = sorted(date for date in normalized_dates.unique().tolist() if start <= date <= end)
    ...
    if not selected_dates:
        ...
        start_idx = next((idx for idx, date in enumerate(sorted(normalized_dates.unique().tolist())) if date >= selected_dates[0]), 0)
        all_dates = sorted(normalized_dates.unique().tolist())
        buffered_start = all_dates[max(0, start_idx - 20)]
        source = source[(normalized_dates >= buffered_start) & (normalized_dates <= selected_dates[-1])].copy()
    features = build_daily_technical_state(source, params=params)
    if selected_dates:
        dates = features['trade_date'].astype(str)
        selected = set(selected_dates)
        features = features[dates.isin(selected)].reset_index(drop=True)
    qa = build_daily_technical_state_qa(features, params=params)
    qa['input_parquet'] = str(Path(args.input_parquet).expanduser())
    qa['partition_column'] = 'trade_date' if args.output_root and args.partitioned else None
    qa['safety'] = {
    ...
    if args.output_root:
        ...
        qa['output_parquet'] = str(output_path)
