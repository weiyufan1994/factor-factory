...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Build intraday_terminal_corr_state_v1 from minute_bar-like parquet.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    params = IntradayTerminalCorrStateParams(
        cutoff_times=tuple(split_csv(args.cutoff_times)),
        windows=tuple(int(item) for item in split_csv(args.windows)),
        min_minutes=int(args.min_minutes),
        research_window=str(args.research_window),
        operator_backend=str(args.operator_backend),
        max_workers=args.max_workers,
    )
    output = derive_intraday_terminal_corr_state(minute, params=params, target_dates=target_dates)
    write_partitioned_datamart(output, output_root)
    runtime_seconds = float(time.perf_counter() - started)
    qa = build_intraday_terminal_corr_state_qa(
        output,
        params=params,
