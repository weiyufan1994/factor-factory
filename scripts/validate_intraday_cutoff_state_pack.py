...
def validate_intraday_cutoff_state_pack(
    *,
    ...
    rebuilt_qa = build_qa_summary(frame, output_path=feature_parquet)
    ...
    if legality.get('uses_full_day_denominator') is not False:
        issues.append('uses_full_day_denominator_not_false')
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Validate intraday_cutoff_state_pack_v1 parquet and warm-read proof.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = validate_intraday_cutoff_state_pack(
