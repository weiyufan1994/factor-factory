...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Validate a plan-only intraday_cutoff_state_pack_v1 worker bundle.')
...
def validate_payload(payload: dict[str, Any], *, bundle_path: Path) -> dict[str, Any]:
    ...
    if payload.get('input_dataset') != 'minute_bar':
        issues.append('input_dataset_must_be_minute_bar')
    if not payload.get('instance_id'):
        issues.append('instance_id_missing')
    if not payload.get('repo'):
        issues.append('repo_missing')
    if not payload.get('minute_root'):
        issues.append('minute_root_missing')
    ...
    if int(payload.get('min_minutes') or 0) <= 0:
        issues.append('min_minutes_must_be_positive')
    if int(payload.get('terminal_window_minutes') or 0) <= 0:
