...
def validate_intraday_ema_slow_state(
    *,
    ...
    rebuilt_qa = build_intraday_ema_slow_state_qa(frame, output_path=feature_parquet)
    ...
    if legality.get('no_future_data') is not True:
        issues.append('no_future_data_missing')
    if 'state_continuity' not in legality:
        issues.append('state_continuity_note_missing')
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Validate intraday_ema_slow_state_v1 parquet and warm-read proof.')
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = validate_intraday_ema_slow_state(
