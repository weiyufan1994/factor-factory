...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Validate a plan-only moneyflow_slow_state_v1 worker resume bundle.')
...
def validate_payload(payload: dict[str, Any], *, bundle_path: Path) -> dict[str, Any]:
    ...
    if payload.get('operator_id') != 'moneyflow_slow_state_v1':
        issues.append('operator_id_not_moneyflow_slow_state_v1')
    if payload.get('evidence_scope') not in {'bounded_worker', 'production_scale', 'full_is'}:
        issues.append('evidence_scope_invalid')
    if payload.get('default_backend') != 'reference':
        issues.append('default_backend_must_be_reference')
