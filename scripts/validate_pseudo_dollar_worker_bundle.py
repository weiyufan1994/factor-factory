...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Validate a plan-only pseudo dollar exploratory worker bundle.')
...
def validate_payload(payload: dict[str, Any], *, bundle_path: Path) -> dict[str, Any]:
    ...
    if payload.get('true_tick_dollar_bar') is not False:
        issues.append('true_tick_dollar_bar_must_be_false')
    if payload.get('source_is_pseudo_from_1m_bar') is not True:
        issues.append('source_is_pseudo_from_1m_bar_must_be_true')
    if payload.get('not_full_window_production_plan') is not True:
        issues.append('not_full_window_production_plan_must_be_true')
    if not payload.get('instance_id'):
        issues.append('instance_id_missing')
    if not payload.get('s3_root'):
        issues.append('s3_root_missing')
    if not payload.get('local_input_dir'):
        issues.append('local_input_dir_missing')

    full_window = payload.get('full_window_contract') or {}
    if full_window.get('unique_key') != ['ts_code', 'trade_date', 'bucket_id']:
