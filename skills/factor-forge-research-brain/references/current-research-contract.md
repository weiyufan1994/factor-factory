# Factor Forge Research Brain Current Research Contract

This reference preserves the complete current normative research-judgment
contract. Read it in full before a serious Step5/6 review, EVO V2 diagnosis,
revision proposal, Council derivation, or promote/iterate/reject judgment.

## Contents

- [Core Philosophy](#core-philosophy)
- [EVO V2 Contradiction and Induction Discipline](#evo-v2-contradiction-and-induction-discipline)
- [Research Quality Claim Ladder](#research-quality-claim-ladder)
- [Current Trading Mandate](#current-trading-mandate)
- [Long-Side Performance Economics](#long-side-performance-economics)
- [Pre-Cost Information First](#pre-cost-information-first)
- [Factor Complexity Penalty](#factor-complexity-penalty)
- [Canonical Return Sources](#canonical-return-sources)
- [Review Checklist](#review-checklist)
- [Revision Principles](#revision-principles)
- [Program Search Policy](#program-search-policy)
- [Math Discipline](#math-discipline)
...
- [Step6 Research Intelligence](#step6-research-intelligence)
- [Mechanism Reasoner And Case Comparator](#mechanism-reasoner-and-case-comparator)
- [Revision Strategist](#revision-strategist)
- [Revision Council Discipline](#revision-council-discipline)

## Core Philosophy

Explore and explain in the order useful to the research question. Preserve the
irreversible dependencies: source understanding before knowledge-informed
formalization, hypothesis/model/measurement and evaluation design frozen before
results, legal information before execution, and adequate evidence before claims.
For replication, retain the author's construction and distinguish reconstruction
assumptions from independent extensions.

At Step5/6 review, consume the frozen economic hypothesis, estimand, information
set, mathematical object, market-outcome map and measurement program. Do not
reselect the model or invent a payer after observing results. Open tool/model
search belongs to initial formalization or an explicit new revision; select only
the audits relevant to that mechanism. Missing pre-result records remain a gap,
not an invitation to backfill them.

Interpret metrics against those predictions and distinguish implementation/data
defects, falsified claims and unidentified explanations before choosing a bounded
revision or stop. Record supported lessons and uncertainty; a new mechanism,
innovation seed or branch is not required as a research outcome. The existing
formal protocol and field constraints remain binding as detailed below.
...
## EVO V2 Contradiction and Induction Discipline

When EVO V2 is enabled, read
`docs/contracts/factorforge-epistemic-evolution-v2.zh-CN.md`. Preserve the
research constitution: do not edit Skills/validators, expand permissions,
change estimand/measurement program/thresholds/OOS/multiplicity/trial budget,
or grant canonical authority. Evolution may change only the research question,
public derivation, transfer mapping and preregistered test proposal.

Diagnose on purged IS before any OOS release. Freeze preferred,
mechanism-distinct alternative and null/alias predictions first. Admit a
qualified contradiction only after lower-layer clearance, information-time
legality, preregistered materiality, multiplicity/trial-budget control, IS
replication and discrimination between rival models. High IC, Sharpe, a large
residual, Council votes, or a historically successful recipe are not triggers.

Act as a contradiction detective, not a high-score hunter. Write a tension
ledger with predicted/observed signatures, mismatch vector, failed layer, at
least two rival explanations, next distinguishing test, `what_survived`, and
`what_failed`. Preserve partial survival: information content may survive while
tradability or after-cost long economics fails.

Apply Dirac-style inversion only to a qualified contradiction. Produce one
`MINIMAL_MECHANISM_DELTA` with a zero-extension recovery test, preserved and
broken invariants, deleted-information audit, unique prediction and complexity
cost, or honestly return `NO_DERIVED_LAW`. Never force DCF, stochastic process,
dimensional analysis, or another preferred family. Backproject any mathematical
delta into actor/action/constraint, payer/receiver, payoff transfer,
persistence/capacity, observable proxy, negative control, counterfactual and
disappearance condition. Without legal proxies and distinguishing tests, keep
it hypothesis-only.

Retrieve memory after blind derivation using a mechanism fingerprint. Rank by
payer/constraint, estimand, mathematical object, invariant/boundary,
observation map and failure signature; never rank by historical return. Treat
market state/events as provenance-bearing stress coordinates, not regime
routers, unless current state dependence was preregistered. Require an actual
before/after question/test change receipt before claiming experience transfer.
...
7. Is a next branch justified, or should the study stop or retain an unresolved question?
...
6. What complexity was added or removed, and is the marginal benefit worth the
   complexity penalty?
7. If a next loop is justified, which bounded search method serves its distinguishing question?
...
For EVO V2, search methods may propose bounded child hypotheses only after a
Host-qualified contradiction. They may not tune the parent on consumed OOS,
select a law by reward/majority vote, or execute a child before external human
approval, a new trial ledger and fresh sealed OOS allocation.
...
Record what the case teaches and the limits of reuse. A case need not produce a
new mechanism or innovation to be worth preserving.
...
Record supported transferable patterns, anti-patterns, actual similar-case
lessons and reuse conditions. Add innovative idea seeds only when warranted.
When no defensible seed or transferable lesson exists, retain the specific reason
and uncertainty instead of manufacturing content. A cold-start knowledge note
must not pretend that a similar case was retrieved.

This research freedom does not relax current consumers: `validate_step6.py`
still requires nonempty `transferable_patterns`, `anti_patterns`,
`similar_case_lessons_imported`, `innovative_idea_seeds` and
`reuse_instruction_for_future_agents`, including ordinary local rejection.
If a case cannot honestly satisfy them, report the concrete unmet field and
formal BLOCK; do not pad lists or claim empty idea-seeds already pass. See
[compatibility constraints](../../factor-forge-ultimate/references/compatibility-constraints.md)
for the unchanged checks and later coordinated consumer/test work.

Express EVO V2 experience in exactly three semantic layers:

- `structural_lesson`: cross-episode payer/constraint, estimand, mathematical
  object, invariant/boundary, observation map, prediction, falsifier,
  counterexample and reuse boundary;
- `conditional_realization`: causal enabling/suppressing/aliasing/challenging
  condition plus diagnostic, interaction signature and falsifier; never a
  bull/bear recipe selector;
- `historical_episode`: immutable Host-signed facts, timeline, state variables,
  predicted-versus-observed signature and layer-by-layer verdict, with no
  normative authority.

Do not canonicalize an empirical `structural_lesson` from one successful case.
Except for strict identity/institutional proof, require at least two independent
episodes, a counterepisode or negative control, invariance audit, independent
review and Host CAS promotion.

Go beyond a pass/fail label: explain supported reuse conditions, counterexamples
and remaining gaps, or why no transferable conclusion can safely be drawn.
...
Compare observed long-side evidence with the frozen return-source hypothesis
and expected mechanism signature. Keep an unidentified return source unresolved;
review need not manufacture an explanation or reselect a model. IC, long-short
spread, or a weak short side cannot establish a mechanism.
