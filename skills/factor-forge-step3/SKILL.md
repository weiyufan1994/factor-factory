...
- first runnable factor values when Step 3A local snapshots are available:
  ...
  - Formula-IR execution defaults to pandas optimized with pandas reference as
    ...
    `std`, `stddev`, `min`, `max`, `delta`, `delay`, `argmin`, `argmax`,
    `ts_rank`, `corr`, `correlation`, and `covariance`; rollback must be
    available through `FACTORFORGE_DISABLE_DEFAULT_NUMPY_TS_KERNEL=1`. The
