...
from factor_factory.measurement_program import research_compatibility_profile_from_spec
...
def report_bound_compatibility_profile(root: Path, report_id: str) -> str | None:
    ...
    if not path.is_file() or path.is_symlink():
        return None
    try:
        return research_compatibility_profile_from_spec(json.loads(path.read_text()))
...
def run_validate_final_research_protocol(
    ...
    report = validate_protocol_bundle(
        ...
        compatibility_profile=report_bound_compatibility_profile(root, report_id),
    ...
    if protocol_required:
        protocol_result = (
            validate_protocol_bundle(
                ...
                compatibility_profile=report_bound_compatibility_profile(root, rid),
