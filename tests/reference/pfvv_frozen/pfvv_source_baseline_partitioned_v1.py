...
class PFVVPartitionState:
    ...
    def retained_daily_state_count(self) -> int:
        return sum(len(values) for values in self.pf_history.values()) + sum(
            len(values) for values in self.vv_z_history.values()
        )
...
def _next_daily_state(
    state: PFVVPartitionState,
    prospective_codes: list[str],
    pf_daily: pd.Series,
    vv_daily_z: pd.Series,
) -> tuple[dict[str, deque[float]], dict[str, deque[float]], pd.Series, pd.Series, pd.Series]:
    """Calculate one day of bounded state without mutating the caller state."""
    next_pf_history: dict[str, deque[float]] = {}
    next_vv_z_history: dict[str, deque[float]] = {}
    pf20: dict[str, float] = {}
    vv20: dict[str, float] = {}
    warmup: dict[str, bool] = {}
    for code in prospective_codes:
        pf_values = deque(state.pf_history.get(code, ()), maxlen=WINDOW)
        vv_values = deque(state.vv_z_history.get(code, ()), maxlen=WINDOW)
        pf_values.append(float(pf_daily.get(code, np.nan)))
        vv_values.append(float(vv_daily_z.get(code, np.nan)))
        next_pf_history[code] = pf_values
        next_vv_z_history[code] = vv_values
        is_warmup = len(pf_values) < WINDOW or len(vv_values) < WINDOW
        warmup[code] = is_warmup
        pf_array = np.asarray(pf_values, dtype=float)
        vv_array = np.asarray(vv_values, dtype=float)
        pf20[code] = float(np.std(pf_array, ddof=1)) if not is_warmup and np.isfinite(pf_array).all() else np.nan
        vv20[code] = float(np.std(vv_array, ddof=1)) if not is_warmup and np.isfinite(vv_array).all() else np.nan
    index = pd.Index(prospective_codes, name="ts_code")
    return (
        next_pf_history,
        next_vv_z_history,
        pd.Series(pf20, index=index, dtype=float),
        pd.Series(vv20, index=index, dtype=float),
