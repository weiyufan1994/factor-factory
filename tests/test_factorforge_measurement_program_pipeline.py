...
def test_step3_uses_frozen_step2_summary_not_component_or_bare_cited_ids() -> None:
    program = _program()
    program["implementation"]["components"][0]["knowledge_node_ids"] = ["retrieved-case", "adopted-method"]
    contract = {
        "contract_version": "factorforge_knowledge_reference_contract_v1",
        "producer": "frozen_retrieval",
        "retrieval_required": False,
        "retrieval_status": "retrieved",
        "query_hash": "a" * 64,
        "indexes_available": ["knowledge/graph/nodes.jsonl"],
        "hit_count": 1,
        "retrieved_case_ids": ["retrieved-case"],
        "retrieved_cases": [{"id": "retrieved-case"}],
    }
    adopted_summary = {
        "nodes": [{"id": "adopted-method"}],
        "related_edges": [],
        "use_phase": "frozen_source_first_advisory",
        "A0_blind_claimed": False,
    }
    fsm = {
        "implementation_mode": "operator",
        "mechanism_conditioned_measurement_program": deepcopy(program),
        "canonical_spec": {"mechanism_conditioned_measurement_program": deepcopy(program)},
        "research_contract": {
            "knowledge_reference_contract": deepcopy(contract),
            "factor_knowledge_context": deepcopy(adopted_summary),
        },
    }
    handoff = {"mechanism_conditioned_measurement_program": deepcopy(program)}
    assert STEP3.validated_measurement_program_for_step3(
        fsm, handoff, implementation_mode="operator",
    ) == program

    forged = deepcopy(fsm)
    for container in (forged, forged["canonical_spec"]):
        container["mechanism_conditioned_measurement_program"]["implementation"]["components"][0]["knowledge_node_ids"] = ["forged-component-id"]
    forged_handoff = {"mechanism_conditioned_measurement_program": deepcopy(forged["mechanism_conditioned_measurement_program"])}
    with pytest.raises(SystemExit, match="knowledge_node_ids_not_in_summary"):
        STEP3.validated_measurement_program_for_step3(
            forged, forged_handoff, implementation_mode="operator",
        )

    missing = deepcopy(fsm)
    del missing["research_contract"]["knowledge_reference_contract"]
    missing_handoff = {"mechanism_conditioned_measurement_program": deepcopy(missing["mechanism_conditioned_measurement_program"])}
    with pytest.raises(SystemExit, match="knowledge_node_ids_not_in_summary"):
        STEP3.validated_measurement_program_for_step3(
            missing, missing_handoff, implementation_mode="operator",
        )

    cold_start = deepcopy(fsm)
    del cold_start["research_contract"]["knowledge_reference_contract"]
    for container in (cold_start, cold_start["canonical_spec"]):
        container["mechanism_conditioned_measurement_program"]["implementation"]["components"][0]["knowledge_node_ids"] = ["adopted-method"]
    cold_start_handoff = {"mechanism_conditioned_measurement_program": deepcopy(cold_start["mechanism_conditioned_measurement_program"])}
    assert STEP3.validated_measurement_program_for_step3(
        cold_start, cold_start_handoff, implementation_mode="operator",
    ) == cold_start["mechanism_conditioned_measurement_program"]

    bare_cited = deepcopy(cold_start)
    del bare_cited["research_contract"]["factor_knowledge_context"]
    bare_cited["knowledge_reference_contract"] = {"cited_node_ids": ["adopted-method"]}
    bare_cited_handoff = {"mechanism_conditioned_measurement_program": deepcopy(bare_cited["mechanism_conditioned_measurement_program"])}
    with pytest.raises(SystemExit, match="knowledge_node_ids_not_in_summary"):
        STEP3.validated_measurement_program_for_step3(
            bare_cited, bare_cited_handoff, implementation_mode="operator",
        )
