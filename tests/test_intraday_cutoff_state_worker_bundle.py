...
def test_cutoff_state_worker_bundle_is_plan_only_and_full_is(tmp_path: Path):
    builder = _load_script('build_intraday_cutoff_state_worker_bundle.py')
    ...
    exit_code = builder.main([
        ...
        '--minute-root',
        '/home/ubuntu/factorforge_data_api_cache/s3_parquet/minute_bar-raw_v1-0b2b836c57d763c6',
        '--output-root',
        '/home/ubuntu/factorforge_data_api_cache/datamarts/intraday_cutoff_state_pack_v1_is',
        '--artifact-dir',
        '/home/ubuntu/factorforge_data_api_cache/proofs/intraday_cutoff_state_pack_v1',
        '--label',
        'full_is_cutoff_state_pack',
        '--start',
        '20160104',
        '--end',
        '20250711',
    ...
    assert payload['dataset_id'] == 'intraday_cutoff_state_pack_v1'
    ...
    assert 'scripts/build_intraday_cutoff_state_pack.py' in command_text
    assert '--minute-root /home/ubuntu/factorforge_data_api_cache/s3_parquet/minute_bar-raw_v1-0b2b836c57d763c6' in command_text
    assert '--research-window IS' in command_text
    assert '--skip-existing' in command_text
    assert '--max-dates' in command_text
    assert '--manifest-output' in command_text
    assert 'scripts/validate_intraday_cutoff_state_pack.py' in command_text
    assert 'scripts/run_data_api_read_smoke.py' in command_text
    assert '--frequency intraday_cutoff' in command_text
...
def test_cutoff_state_worker_bundle_validator_accepts_safe_bundle(tmp_path: Path):
    builder = _load_script('build_intraday_cutoff_state_worker_bundle.py')
    validator = _load_script('validate_intraday_cutoff_state_worker_bundle.py')
    ...
    assert builder.main([
        ...
        '/cache/minute_bar',
        '--output-root',
        '/cache/intraday_cutoff_state_pack_v1_is',
        '--artifact-dir',
        '/cache/proofs',
        '--output-path',
        str(bundle_path),
...
def test_cutoff_state_worker_bundle_validator_blocks_remote_dispatch(tmp_path: Path):
    builder = _load_script('build_intraday_cutoff_state_worker_bundle.py')
    validator = _load_script('validate_intraday_cutoff_state_worker_bundle.py')
    bundle_path = tmp_path / 'bundle.json'
    validation_path = tmp_path / 'validation.json'
