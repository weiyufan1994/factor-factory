...
def test_optional_model_layer_review_is_closed_and_negative_only():
    valid_review = {
        ...
        "interpretation": "The current EVENT_U diagnosis is challenged; no causal identification is claimed.",
    }
    assert memo_public_schema_failures({"evidence_comparison": {}}) == []
    assert memo_public_schema_failures(
        {"evidence_comparison": {"model_layer_review": valid_review}}
    ) == []
    assert (
        "BLOCK_MAIN_AGENT_MECHANISM_MEMO_PUBLIC_FIELD_TYPE:"
        "evidence_comparison.model_layer_review"
        in memo_public_schema_failures(
            {"evidence_comparison": {"model_layer_review": None}}
        )
    )

    extra = copy.deepcopy(valid_review)
    extra["nested_payload"] = {"promotion": True}
    assert any(
        "evidence_comparison.model_layer_review.nested_payload" in failure
        for failure in memo_public_schema_failures(
            {"evidence_comparison": {"model_layer_review": extra}}
        )
    )

    missing = copy.deepcopy(valid_review)
    missing.pop("return_source")
    assert (
        "BLOCK_MAIN_AGENT_MECHANISM_MEMO_MODEL_LAYER_REVIEW_FIELD_MISSING:"
        "return_source"
        in memo_public_schema_failures(
            {"evidence_comparison": {"model_layer_review": missing}}
        )
    )

    alternate_negative_layer = copy.deepcopy(valid_review)
    alternate_negative_layer["failed_equation_component"] = "observable_estimator"
    assert memo_public_schema_failures(
        {"evidence_comparison": {"model_layer_review": alternate_negative_layer}}
    ) == []

    invalid_layer = copy.deepcopy(valid_review)
    invalid_layer["failed_equation_component"] = "unknown"
    assert any(
        failure.endswith("failed_equation_component")
        for failure in memo_public_schema_failures(
            {"evidence_comparison": {"model_layer_review": invalid_layer}}
        )
    )

    positive = copy.deepcopy(valid_review)
    positive["mechanism_fit"] = "supported"
    assert any(
        failure.endswith("mechanism_fit")
        for failure in memo_public_schema_failures(
            {"evidence_comparison": {"model_layer_review": positive}}
        )
    )

    explicit_denial = copy.deepcopy(valid_review)
    explicit_denial["interpretation"] = "This is not official promotion."
    assert memo_public_schema_failures(
        {"evidence_comparison": {"model_layer_review": explicit_denial}}
    ) == []
